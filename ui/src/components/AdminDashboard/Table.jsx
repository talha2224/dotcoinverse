...
const Table = ({ title, showLeftSideBar, setshowLeftSideBar, th, data }) => {
    ...
    const approvedPayment = async (id) => {
        axios.put(`/api/v1/user/deposit/${id}`,{approved:true})
            .then((res) => {
                if (res.status === 200) {
    ...
    const declinePayment = async (id) => {
        axios.put(`/api/v1/user/deposit/${id}`,{approved:false})
        .then((res) => {
                if (res.status === 200) {
    ...
    return (
        <div className={`font-roboto w-[100%] h-[100%] flex flex-col p-5 overflow-y-auto`}>
            ...
            <div>
                <input onChange={(e) => setSearchValue(e.target.value)} style={{ border: "0.1px solid lightgray" }} type="text" placeholder='Search' className='w-[18rem] placeholder:text-sm h-[2.5rem] rounded-md pl-4 pr-4 mt-3 outline-none ' />
            </div>
                                            ...
                                            return (
                                                <tr key={item._id} className='cursor-pointer'>
                                                    ...
                                                    <td className='p-4 text-sm text-gray-600  whitespace-nowrap border-2 border-[#E9EBED] tracking-wider'>
                                                        <a href={item.image} download>{item.image ? "Download" : "Not Uploaded"}</a>
                                                    </td>
                                                    {
                                                        item.decline === true?
