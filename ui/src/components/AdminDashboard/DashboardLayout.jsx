...
import userLinks from '../../constants/adminLink';
        ...
        <div className='mt-7'>
          {
            userLinks?.map((item, index) => {
              return (
                <div onClick={()=>onNavClick(item.id,item.link)} key={index + item?.id} className='hover:bg-[#5b6aa1] hover:text-white text-[#8e99c1] h-[2rem] mb-5 rounded-sm cursor-pointer flex justify-start gap-3 pl-2 items-center'>
                  <p>{item?.icon}</p>
