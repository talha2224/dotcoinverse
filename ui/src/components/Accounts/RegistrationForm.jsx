...
const RegistrationForm = () => {
  ...
  const SubmitForm = async(e) => {
    e.preventDefault()
    setloading(true)
    if (!userData.email || !userData.password || !userData.firstname || !userData.lastname || !userData.refferedBy) {
      setloading(false)
  ...
  return (
    ...
    <div data-aos='zoom-in' className={`font-roboto w-[100%] pl-10 pr-10 h-[85vh] overflow-y-auto `}>

      <div className='flex justify-center items-center h-[100%] pb-4'>

        <div className='pt-4 pb-4 pl-8 pr-8 h-[25rem] overflow-y-auto bg-[#0e1734] shadow-formShadow rounded-md'>
          ...
          <div className='mt-10'>
            ...
            {/* REFFERAL CODE  */}
            <div className='mt-3'>
              <input onChange={onChangeInput} style={{ border: "1.5px solid #888" }} type="text" name="refferedBy" placeholder='Referal Code' className='w-[100%] h-[2.5rem]  outline-none placeholder:text-sm placeholder:tracking-widest rounded-md pl-4 pr-4 bg-transparent text-[#888]' />
            </div>
            ...
            <div className='mt-10'>
              <p className='text-white text-sm tracking-wider'>Already have an account? <Link to='/account/login' className='text-[#888]'>Login Now</Link></p>
            </div>
